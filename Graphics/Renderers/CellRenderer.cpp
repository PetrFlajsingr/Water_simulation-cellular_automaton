...
CellRenderer::CellRenderer(const std::string cellModelPath, const glm::mat4 &projectionMat, const glm::uvec3 &tankSize)
  ...
  vao->addAttrib(vbo, 2, 2, GL_FLOAT, static_cast<GLsizei>(sizeof(Model::VertexData)), offsetof(Model::VertexData, texCoord));
...
const std::shared_ptr<Buffer> &CellRenderer::getIbo() const { return ibo; }

const std::shared_ptr<Buffer> &CellRenderer::getPositionsBuffer() const { return positionsBuffer; }
