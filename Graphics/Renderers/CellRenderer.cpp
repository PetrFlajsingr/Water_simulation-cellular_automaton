...
CellRenderer::CellRenderer([[maybe_unused]] const std::string &cellModelPath, const glm::mat4 &projectionMat,
  ...
  basicProgram = std::make_shared<Program>("basic"_vert, "basic"_frag, "basic"_geom);
  velocityProgram = std::make_shared<Program>("basic"_vert, "basic"_frag, "velocity"_geom);
...
void CellRenderer::draw(const glm::mat4 &viewMat, const glm::vec3 &cameraPos, const BufferPtr &cellBuffer,
                        const BufferPtr &cellInfoBuffer, unsigned int simulationType, float cellSize) {
  ...
  if (simulationType == 0) {
    ...
    basicProgram->setMatrix4fv("Model", glm::value_ptr(glm::mat4(1.0)), 1, GL_FALSE);
    basicProgram->setMatrix4fv("View", glm::value_ptr(viewMat), 1, GL_FALSE);
    basicProgram->setMatrix4fv("Projection", glm::value_ptr(projectionMat), 1, GL_FALSE);
    basicProgram->set("cellSize", cellSize);
    basicProgram->set("time", t);
    basicProgram->set3uiv("tankSize", glm::value_ptr(tankSize), 1);
    basicProgram->set3fv("cameraPosition", glm::value_ptr(cameraPos));
    ...
    cellBuffer->bindBase(GL_SHADER_STORAGE_BUFFER, 1);
    cellInfoBuffer->bindBase(GL_SHADER_STORAGE_BUFFER, 2);
    velocityProgram->setMatrix4fv("Model", glm::value_ptr(glm::mat4(1.0)), 1, GL_FALSE);
    velocityProgram->setMatrix4fv("View", glm::value_ptr(viewMat), 1, GL_FALSE);
    velocityProgram->setMatrix4fv("Projection", glm::value_ptr(projectionMat), 1, GL_FALSE);
    velocityProgram->set("cellSize", cellSize);
    velocityProgram->set("time", t);
    velocityProgram->set3uiv("tankSize", glm::value_ptr(tankSize), 1);
    velocityProgram->set3fv("cameraPosition", glm::value_ptr(cameraPos));
    glDrawArraysInstanced(GL_POINTS, 0, 1, glm::compMul(tankSize));
    cellBuffer->unbindBase(GL_SHADER_STORAGE_BUFFER, 1);
    cellInfoBuffer->unbindBase(GL_SHADER_STORAGE_BUFFER, 2);
    vao->unbind();
