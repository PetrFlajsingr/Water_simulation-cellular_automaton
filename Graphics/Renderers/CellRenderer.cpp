...
CellRenderer::CellRenderer([[maybe_unused]] const std::string &cellModelPath, const glm::mat4 &projectionMat, const glm::uvec3 &tankSize)
  ...
  vbo = createBuffer(std::vector<Model::VertexData>{Model::VertexData{glm::vec3(0.0), glm::vec3{0.0,0.0,1.0}, glm::vec2(0.0), glm::vec3(0.0)}});
...
void CellRenderer::draw(const glm::mat4 &viewMat, const glm::vec3 &cameraPos, const BufferPtr& cellBuffer, float cellSize) {
  program->use();
