...
template <precision P> struct compute_dot<tquat, float, P, true> {
  static GLM_FUNC_QUALIFIER float call(tquat<float, P> const &x, tquat<float, P> const &y) {
    return _mm_cvtss_f32(glm_vec1_dot(x.data, y.data));
  }
};

template <precision P> struct compute_quat_add<float, P, true> {
...
template <precision P> struct compute_quat_add<double, P, true> {
...
template <precision P> struct compute_quat_sub<float, P, true> {
  static tquat<float, P> call(tquat<float, P> const &q, tquat<float, P> const &p) {
...
#if GLM_ARCH & GLM_ARCH_AVX_BIT
template <precision P> struct compute_quat_sub<double, P, true> {
  static tquat<double, P> call(tquat<double, P> const &a, tquat<double, P> const &b) {
    tquat<double, P> Result(uninitialize);
...
template <precision P> struct compute_quat_mul_scalar<float, P, true> {
...
#if GLM_ARCH & GLM_ARCH_AVX_BIT
template <precision P> struct compute_quat_mul_scalar<double, P, true> {
...
template <precision P> struct compute_quat_div_scalar<float, P, true> {
  static tquat<float, P> call(tquat<float, P> const &q, float s) {
    tvec4<float, P> Result(uninitialize);
...
#if GLM_ARCH & GLM_ARCH_AVX_BIT
template <precision P> struct compute_quat_div_scalar<double, P, true> {
  static tquat<double, P> call(tquat<double, P> const &q, double s) {
    tquat<double, P> Result(uninitialize);
...
template <precision P> struct compute_quat_mul_vec4<float, P, true> {
