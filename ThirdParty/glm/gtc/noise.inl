...
namespace glm {
namespace gtc {
template <typename T, precision P> GLM_FUNC_QUALIFIER tvec4<T, P> grad4(T const &j, tvec4<T, P> const &ip) {
...
template <typename T, precision P> GLM_FUNC_QUALIFIER T perlin(tvec2<T, P> const &Position) {
...
// Classic Perlin noise
template <typename T, precision P> GLM_FUNC_QUALIFIER T perlin(tvec3<T, P> const &Position) {
...
*/
// Classic Perlin noise
template <typename T, precision P> GLM_FUNC_QUALIFIER T perlin(tvec4<T, P> const &Position) {
  ...
  tvec4<T, P> norm00 =
      detail::taylorInvSqrt(tvec4<T, P>(dot(g0000, g0000), dot(g0100, g0100), dot(g1000, g1000), dot(g1100, g1100)));
  g0000 *= norm00.x;
  g0100 *= norm00.y;
  g1000 *= norm00.z;
  g1100 *= norm00.w;
  ...
  tvec4<T, P> norm01 =
      detail::taylorInvSqrt(tvec4<T, P>(dot(g0001, g0001), dot(g0101, g0101), dot(g1001, g1001), dot(g1101, g1101)));
  g0001 *= norm01.x;
  g0101 *= norm01.y;
  g1001 *= norm01.z;
  g1101 *= norm01.w;
  ...
  tvec4<T, P> norm10 =
      detail::taylorInvSqrt(tvec4<T, P>(dot(g0010, g0010), dot(g0110, g0110), dot(g1010, g1010), dot(g1110, g1110)));
  g0010 *= norm10.x;
  g0110 *= norm10.y;
  g1010 *= norm10.z;
  g1110 *= norm10.w;
  ...
  tvec4<T, P> norm11 =
      detail::taylorInvSqrt(tvec4<T, P>(dot(g0011, g0011), dot(g0111, g0111), dot(g1011, g1011), dot(g1111, g1111)));
  g0011 *= norm11.x;
  g0111 *= norm11.y;
  g1011 *= norm11.z;
  g1111 *= norm11.w;
...
template <typename T, precision P> GLM_FUNC_QUALIFIER T perlin(tvec2<T, P> const &Position, tvec2<T, P> const &rep) {
...
// Classic Perlin noise, periodic variant
template <typename T, precision P> GLM_FUNC_QUALIFIER T perlin(tvec3<T, P> const &Position, tvec3<T, P> const &rep) {
...
template <typename T, precision P> GLM_FUNC_QUALIFIER T simplex(glm::tvec2<T, P> const &v) {
  tvec4<T, P> const C = tvec4<T, P>(T(0.211324865405187),  // (3.0 -  sqrt(3.0)) / 6.0
  ...
  tvec3<T, P> p = detail::permute(detail::permute(i.y + tvec3<T, P>(T(0), i1.y, T(1))) + i.x + tvec3<T, P>(T(0), i1.x, T(1)));

  tvec3<T, P> m = max(tvec3<T, P>(0.5) - tvec3<T, P>(dot(x0, x0), dot(tvec2<T, P>(x12.x, x12.y), tvec2<T, P>(x12.x, x12.y)),
                                                     dot(tvec2<T, P>(x12.z, x12.w), tvec2<T, P>(x12.z, x12.w))),
                      tvec3<T, P>(0));
  m = m * m;
  m = m * m;
...
template <typename T, precision P> GLM_FUNC_QUALIFIER T simplex(tvec3<T, P> const &v) {
  ...
  // Permutations
  i = detail::mod289(i);
  tvec4<T, P> p(detail::permute(
      detail::permute(detail::permute(i.z + tvec4<T, P>(T(0), i1.z, i2.z, T(1))) + i.y + tvec4<T, P>(T(0), i1.y, i2.y, T(1))) +
  ...
  tvec4<T, P> a0 =
      tvec4<T, P>(b0.x, b0.z, b0.y, b0.w) + tvec4<T, P>(s0.x, s0.z, s0.y, s0.w) * tvec4<T, P>(sh.x, sh.x, sh.y, sh.y);
  tvec4<T, P> a1 =
      tvec4<T, P>(b1.x, b1.z, b1.y, b1.w) + tvec4<T, P>(s1.x, s1.z, s1.y, s1.w) * tvec4<T, P>(sh.z, sh.z, sh.w, sh.w);
...
template <typename T, precision P> GLM_FUNC_QUALIFIER T simplex(tvec4<T, P> const &v) {
  tvec4<T, P> const C(0.138196601125011,   // (5 - sqrt(5))/20  G4
  ...
  tvec4<T, P> j1 = detail::permute(detail::permute(detail::permute(detail::permute(i.w + tvec4<T, P>(i1.w, i2.w, i3.w, T(1))) +
  ...
  m1 = m1 * m1;
  return T(49) *
         (dot(m0 * m0, tvec3<T, P>(dot(p0, x0), dot(p1, x1), dot(p2, x2))) + dot(m1 * m1, tvec2<T, P>(dot(p3, x3), dot(p4, x4))));
}
} // namespace glm
